...
fn parse_key_event_record(key_event: &KeyEventRecord) -> Option<WindowsKeyEvent> {
    ...
    // Letter virtual-key codes describe physical shortcut keys independently
    // of the active keyboard layout. Prefer them for Control shortcuts, while
    // retaining the layout-resolved Unicode character for normal typing.
    let physical_control_character = if modifiers.contains(KeyModifiers::CONTROL)
        && !modifiers.contains(KeyModifiers::ALT)
        && (0x41..=0x5A).contains(&virtual_key_code)
    {
        char::from_u32((virtual_key_code - 0x41) as u32 + u32::from(b'a'))
    } else {
        None
    };

    let parse_result =
        physical_control_character
            .map(KeyCode::Char)
            .or_else(|| match virtual_key_code {
