...
pub(crate) fn parse_csi_u_encoded_key_code(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    ...
    // The Kitty protocol reports both the shifted character and the base-layout
    // character. Use the latter for Control shortcuts so they continue to refer
    // to the same physical key after the user changes keyboard layout. Keep the
    // layout-resolved character for ordinary text input.
    if modifiers.contains(KeyModifiers::CONTROL) && !modifiers.contains(KeyModifiers::ALT) {
        if let Some(base_layout_c) = base_layout_codepoint {
            keycode = KeyCode::Char(base_layout_c);
        }
...
mod tests {
    ...
    #[test]
    fn test_parse_csi_u_control_shortcut_uses_base_layout_keycode() {
        assert_eq!(
            // Ctrl+C on the physical C key while the Russian layout is active.
            parse_event(b"\x1B[1089:1057:99;5u", false).unwrap(),
            Some(InternalEvent::Event(Event::Key(KeyEvent::new(
                KeyCode::Char('c'),
                KeyModifiers::CONTROL,
            )))),
    ...
    #[test]
    fn test_parse_csi_u_plain_input_keeps_current_layout_keycode() {
        assert_eq!(
            // Plain input must still insert the Russian character.
            parse_event(b"\x1B[1089:1057:99;1u", false).unwrap(),
            Some(InternalEvent::Event(Event::Key(KeyEvent::new(
                KeyCode::Char('с'),
                KeyModifiers::empty(),
            )))),
